    ...
    <nav className={`navbar-container ${isScrolled ? 'scrolled' : ''} ${isOpen ? 'menu-open' : ''}`}>
      <Link href="/" className="nav-logo" onClick={() => setIsOpen(false)}>
        <svg className="wow-logo-svg" viewBox="0 0 160 40" width="120" height="30" fill="currentColor">
          ...
          <path className="logo-book-pages" d="M12,12 L17,25 L23,17 L24,17 L30,25 L35,12" fill="none" stroke="var(--bg-base)" strokeWidth="1.5" strokeLinecap="round" />
      ...
      {/* Far right control group for mobile containing theme toggle + burger */}
      <div className="flex md:hidden items-center gap-4 z-[220]">
        <button className="theme-toggle-btn" onClick={toggleTheme} aria-label="Toggle Theme">
          {theme === 'dark' ? (
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5">
              <circle cx="12" cy="12" r="5" />
